...
- I used some external commands:
    ...
    - `awk '{print $2" "$1}'`: where `$1, $2` are first and second columns respectively, since `uniq` returns frequency first then the term, so this  was for formatting the output.
